...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        [arr enumerateObjectsUsingBlock:^(TRImageView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [obj.imageView setImageWithURL:_icons[idx] placeholderImage:[UIImage imageNamed:@"account_mars_default"]];
        }];
        return cell;
    }else{
        BestGroupDetailHeroCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HeroCell"];
        [cell.iconView.imageView setImageWithURL:_icons[indexPath.row] placeholderImage:[UIImage imageNamed:@"account_mars_default"]];
